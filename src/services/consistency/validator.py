...
from src.models.consistency_rule import ConsistencyRule, RuleSeverity, RuleType, RuleScope, ValidationLogic
...
class ConsistencyValidator:
    ...
    def validate(self, story_elements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            timeline_issues, timeline_strengths = self._validate_timeline(story_elements.get("events", []))
            issues.extend(timeline_issues)
            strengths.extend(timeline_strengths)

            # Validate character consistency
            character_issues, character_strengths = self._validate_characters(story_elements.get("characters", []))
            ...
            plot_issues, plot_strengths = self._validate_plot_consistency(story_elements)
            ...
            self.logger.info(f"Consistency validation complete. Score: {overall_score:.2f}, Confidence: {confidence_score:.2f}")
    ...
    def _validate_timeline(self, events: List[Dict[str, Any]]) -> Tuple[List[Dict], List[str]]:
        ...
        for i in range(len(events) - 1):
            ...
            if "timestamp" in event1 and "timestamp" in event2:
                if self._compare_timestamps(event1["timestamp"], event2["timestamp"]) > 0:
                    issues.append({
                        "type": "timeline",
                        "severity": "critical",
        ...
        # Identify strengths
        if len(events) > 1 and not any(issue["severity"] == "critical" for issue in issues):
    ...
    def _validate_characters(self, characters: List[Dict[str, Any]]) -> Tuple[List[Dict], List[str]]:
        """Validate character consistency."""
        issues = []
        strengths = []
        ...
        protagonist_count = sum(1 for char in characters if char.get("role") == "protagonist")
    ...
    def _validate_world_rules(self, world_details: List[Dict[str, Any]], events: List[Dict[str, Any]]) -> Tuple[List[Dict], List[str]]:
        ...
        for i, event in enumerate(events):
            ...
            if "jurisdiction" in world_rules and location:
                ...
                if "outside_jurisdiction" in location.lower() and "arrest" in description:
                    issues.append({
                        "type": "world",
                        "severity": "warning",
        ...
        # Identify strengths
        if world_rules:
            strengths.append(f"World building includes {len(world_rules)} consistency rules")
    ...
    def _validate_plot_consistency(self, story_elements: Dict[str, Any]) -> Tuple[List[Dict], List[str]]:
            ...
            for trigger, expected_consequences in consequence_keywords.items():
                if trigger in description:
                    ...
                    for j in range(i + 1, min(i + 5, len(events))):  # Check next 4 events
                        ...
                        if any(consequence in next_event for consequence in expected_consequences):
        ...
        try:
            ...
            if isinstance(timestamp1, str) and isinstance(timestamp2, str):
                ...
                if "_" in timestamp1 and "_" in timestamp2:
                    ...
                    return -1 if time1_val < time2_val else (1 if time1_val > time2_val else 0)
            ...
            return -1 if str(timestamp1) < str(timestamp2) else (1 if str(timestamp1) > str(timestamp2) else 0)
    ...
    def _generate_recommendations(self, issues: List[Dict], story_elements: Dict[str, Any]) -> List[str]:
        ...
        if issue_counts.get("timeline", 0) > 0:
            recommendations.append("Review timeline consistency - ensure events occur in logical chronological order")

        if issue_counts.get("character", 0) > 0:
            recommendations.append("Standardize character attributes and ensure consistency across all appearances")

        if issue_counts.get("world", 0) > 0:
            recommendations.append("Review world-building rules and ensure all events adhere to established constraints")

        if issue_counts.get("plot", 0) > 0:
            recommendations.append("Address plot inconsistencies and ensure cause-and-effect relationships are clear")
        ...
        if len(events) < 3:
            recommendations.append("Consider adding more events to develop the plot structure")

        if len(characters) < 2:
            recommendations.append("Consider adding more characters to create richer interactions")

        # Critical issue recommendations
        critical_issues = [issue for issue in issues if issue.get("severity") == "critical"]
        if critical_issues:
            recommendations.insert(0, f"Address {len(critical_issues)} critical consistency issues immediately")
    ...
    def _calculate_confidence_score(self, issues: List[Dict], story_elements: Dict[str, Any]) -> float:
