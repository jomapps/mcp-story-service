...
from src.models.story_arc import StoryArc, ActStructure, Act, TurningPoint, PacingProfile, ArcStatus
...
class NarrativeAnalyzer:
    ...
    def _get_default_patterns(self) -> Dict:
        ...
        return {
            "acts": {
                "act_one": {"percentage": 25, "key_elements": ["Inciting incident", "Character introduction"]},
                "act_two": {"percentage": 50, "key_elements": ["Rising action", "Midpoint reversal"]},
                "act_three": {"percentage": 25, "key_elements": ["Climax", "Resolution"]}
            },
            "beats": [
    ...
    def analyze_story_structure(self, story_content: str, genre: str) -> StoryArc:
        ...
        try:
            ...
            act_analysis = self._analyze_three_act_structure(story_segments, story_beats)
            ...
            confidence_score = self._calculate_structure_confidence(act_analysis, story_beats, turning_points)
            ...
            self.logger.info(f"Story structure analysis complete. Confidence: {confidence_score:.2f}")
            return story_arc
    ...
    def _identify_story_beats(self, story_content: str, segments: List[str]) -> Dict[str, Dict]:
            ...
            for beat_name, patterns in beat_patterns.items():
                for pattern in patterns:
                    if re.search(pattern, segment_lower):
                        if beat_name not in beats or abs(position - self._get_expected_position(beat_name)) < abs(beats[beat_name]["position"] - self._get_expected_position(beat_name)):
                            beats[beat_name] = {
                                "position": position,
                                "segment_index": i,
                                "content": segment[:200] + "..." if len(segment) > 200 else segment,
                                "confidence": self._calculate_beat_confidence(beat_name, position, segment)
                            }
    ...
    def _calculate_beat_confidence(self, beat_name: str, position: float, content: str) -> float:
        """Calculate confidence score for a detected beat."""
        expected_pos = self._get_expected_position(beat_name)
        position_penalty = abs(position - expected_pos) * 2  # Penalty for wrong position
    ...
    def _analyze_three_act_structure(self, segments: List[str], beats: Dict) -> ActStructure:
        ...
        act_one = Act(
            ...
            key_events=self._extract_act_events(segments, 0, int(act1_end * total_segments), beats),
            character_arcs=[]
        )
        ...
        act_two = Act(
            ...
            key_events=self._extract_act_events(segments, int(act1_end * total_segments), int(act2_end * total_segments), beats),
            character_arcs=[]
        )
        ...
        act_three = Act(
            ...
            key_events=self._extract_act_events(segments, int(act2_end * total_segments), total_segments, beats),
            character_arcs=[]
        )

        # Calculate structure confidence
        structure_confidence = self._calculate_act_confidence(act_one, act_two, act_three, beats)
    ...
    def _extract_act_events(self, segments: List[str], start_idx: int, end_idx: int, beats: Dict) -> List[str]:
        ...
        for beat_name, beat_data in beats.items():
            ...
            if start_idx <= segment_idx < end_idx:
                events.append(f"{beat_name.replace('_', ' ').title()}: {beat_data['content'][:100]}...")
        ...
        if not events and end_idx > start_idx:
            for i in range(start_idx, min(end_idx, len(segments))):
                ...
                if re.search(r'\b(discover|find|realize|decide|fight|escape|meet|arrive|leave)\b', segment.lower()):
                    events.append(segment[:100] + "..." if len(segment) > 100 else segment)
    ...
    def _identify_turning_points(self, beats: Dict) -> List[TurningPoint]:
        ...
        if not turning_points:
            turning_points = [
                TurningPoint(position=0.25, description="First turning point (estimated)"),
                TurningPoint(position=0.75, description="Second turning point (estimated)")
            ]
    ...
    def _calculate_segment_tension(self, segment: str, position: float, beats: Dict) -> float:
        ...
        tension_words = {
            "high": ["fight", "battle", "danger", "threat", "crisis", "attack", "death", "kill"],
            "medium": ["conflict", "argue", "problem", "worry", "fear", "concern", "chase"],
    ...
    def _smooth_tension_curve(self, curve: List[float], target_length: int) -> List[float]:
    ...
    def _analyze_pacing_issues(self, tension_curve: List[float]) -> Tuple[List[str], List[str]]:
    ...
    def _calculate_pacing_confidence(self, tension_curve: List[float], issues: List[str]) -> float:
        ...
        if tension_curve and len(tension_curve) > 3:
            ...
            start_avg = sum(tension_curve[:len(tension_curve)//3]) / (len(tension_curve)//3)
            middle_avg = sum(tension_curve[len(tension_curve)//3:2*len(tension_curve)//3]) / (len(tension_curve)//3)
            end_avg = sum(tension_curve[2*len(tension_curve)//3:]) / (len(tension_curve) - 2*len(tension_curve)//3)
    ...
    def _calculate_act_confidence(self, act1: Act, act2: Act, act3: Act, beats: Dict) -> float:
        ...
        if 0.2 <= act1_length <= 0.3 and 0.4 <= act2_length <= 0.6 and 0.2 <= act3_length <= 0.3:
            base_confidence += 0.1
    ...
    def _calculate_structure_confidence(self, act_structure: ActStructure, beats: Dict, turning_points: List[TurningPoint]) -> float:
        ...
        beat_confidence = sum(beat["confidence"] for beat in beats.values()) / len(beats) if beats else 0.5
        turning_point_confidence = 0.8 if len(turning_points) >= 2 else 0.5

        # Weighted average
        overall_confidence = (act_confidence * 0.5 + beat_confidence * 0.3 + turning_point_confidence * 0.2)
